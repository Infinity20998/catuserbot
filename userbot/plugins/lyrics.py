...
async def song(event):
    ...
    tap = await event.client.inline_query(bot, lyrics)
...
CMD_HELP.update(
    {
        ...
        \n\n**Syntax : **`.lyrics <aritst name - song name>` __or__ `.lyrics <song_name>`\
        \n**Function : ** __searches a song's lyrics and sends you if song name doesn't' work, try along with artist name.__\
        \n\n**Syntax : **`.ilyrics <aritst name - song name>` __or__ `.ilyrics <song_name>`\
        \n**Function : ** __searches a song's lyrics and sends via inline of @iLyricsBot.\n If song name doesn't work, try along with artist name.__\
        ...
        \nAdd:-  `GENIUS_API_TOKEN` and token value in heroku app settings for functioning of glyrics.\
    "
    }
)
