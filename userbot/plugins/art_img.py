...
async def bad(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    output_file = os.path.join("./temp", "iascii.jpg")
    output = await _cattools.media_to_pic(event, reply_message)
    outputt = convert_toimage(output[1], filename="./temp/iascii.jpg")
...
async def pussy(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    output_file = os.path.join("./temp", "line.jpg")
    output = await _cattools.media_to_pic(event, reply_message)
    outputt = convert_toimage(output[1], filename="./temp/line.jpg")
...
async def cat(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    output_file = os.path.join("./temp", "clip.jpg")
    output = await _cattools.media_to_pic(event, reply_message)
    outputt = convert_toimage(output[1], filename="./temp/clip.jpg")
    await cat.delete()
